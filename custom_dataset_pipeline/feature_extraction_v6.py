"""
肌肉特征提取脚本 (版本 v6)
基于 feature_extraction_v5.py 扩展，主要改进：

1. **脂肪阈值计算优化**（适应 CSF 标准化后的新信号分布）：
   - 新增像素值范围裁剪：[0.05, 2.0]，剔除物理上不可能的极端噪声值
   - 双峰检验失败时使用固定回退阈值 **0.6**（而非原中位数+1.5IQR）
     原理：CSF=1.0 标准化后，纯肌肉信号通常 < 0.5，纯脂肪信号通常 > 0.7，
     0.6 恰好在两者鸿沟正中间，是物理意义明确的安全截断值

2. **与 image_preprocessing_v6.py 协同工作**：
   - 输入图像为 CSF 标准化后的图像（CSF 信号 ≈ 1.0）
   - 所有灰度相关特征（FIP、平均强度等）基于新的参考基准

提取的特征包括：
1. 基础形态学特征（13项）
2. 灰度/信号特征（14项）
3. 空间分布特征（8项）
4. 纹理特征（12项，需要 PyRadiomics）
...
输出文件：
- muscle_features_2d_v6.csv: 2D切片级特征（每切片每肌肉一行）
- muscle_features_3d_v6.csv: 3D病人级特征
- muscle_features_level3_cross_v6.csv: Level 3.1跨层梯度特征
- muscle_features_level3_multi_v6.csv: Level 3.2-3.5多肌肉关系特征
- extraction_report_v6.txt: 特征提取检测报告
- feature_extraction_log_v6.json: 批量处理日志（记录每位病人的阈值决策细节）
"""
...
def is_bimodal_valid(pix_vals, min_samples=30):
    """
    检验双峰性。返回 (是否有效, 推荐阈值或 None)

    两种互补的检验，任意一个通过即认为"双峰有效"：
    - 检验 A: 峰谷比 (Peak-to-Valley Ratio)
    - 检验 B: Ashman's D
    """
...
def get_fat_threshold_per_muscle(pix_vals: np.ndarray) -> Tuple[float, str]:
    """
    返回脂肪阈值（CSF 标准化后的版本）

    **v6 关键改进**：
    1. 新增像素值范围裁剪 [0.05, 2.0]，剔除物理上不可能的极端噪声
    2. 双峰检验失败时使用固定回退阈值 **0.6**（而非原中位数+1.5IQR）
       物理依据：CSF=1.0 标准化后，肌肉通常 < 0.5，脂肪通常 > 0.7，0.6 为安全中点

    算法流程：
    1. 像素值范围裁剪 [0.05, 2.0]
    2. 像素数过少 → 返回固定 0.6 并标记 'too_few_pixels'
    3. 检验双峰性
    4. 若双峰有效 → 使用 Otsu 阈值（并约束在 [0.3, 0.9] 范围内），标记 'otsu'
    5. 若双峰无效 → 使用固定阈值 0.6，标记 'fixed_0.6_fallback'

    Args:
        pix_vals: 肌肉区域内的像素值（CSF 标准化后，CSF ≈ 1.0）

    Returns:
        (脂肪阈值, 决策类型标签)
    """
    # ---- v6 改进 1: 像素值范围裁剪 ----
    # 剔除物理上不可能的极端值 (< 0.05 或 > 2.0)
    # CSF=1.0 标准化后，正常组织信号应在 0~2 范围内
    valid_mask = (pix_vals > 0.05) & (pix_vals < 2.0)
    valid = pix_vals[valid_mask]

    decision_type = 'fixed_0.6_fallback'  # 默认决策
    thresh = 0.6

    if len(valid) < 10:
        # 像素太少，无法进行有效统计
        return thresh, 'too_few_pixels'

    # ---- 双峰检验 + Otsu 阈值 ----
    bimodal, _ = is_bimodal_valid(valid)
    if bimodal:
        try:
            otsu_thresh = threshold_otsu(valid)
            # 对 Otsu 结果做合理性约束
            # 在 CSF=1.0 标准化体系下，脂肪肌肉分界应在合理范围内
            low_bound = 0.3   # 低于此值不太可能是脂肪
            high_bound = 0.9  # 高于此值可能是 CSF 污染
            if low_bound <= otsu_thresh <= high_bound:
                thresh = float(otsu_thresh)
                decision_type = 'otsu'
            else:
                # Otsu 给出的阈值超出合理范围，怀疑直方图异常
                bimodal = False
        except:
            bimodal = False
    ...
    # ---- v6 改进 2: 固定回退阈值 0.6 ----
    if not bimodal:
        # 情况：肌肉几乎纯脂肪、几乎纯肌肉，或单峰水肿
        # 不再使用"中位数 + 1.5IQR"这种依赖数据分布的方法
        # 改用物理意义明确的固定阈值 0.6
        thresh = 0.6
        decision_type = 'fixed_0.6_fallback'

    return thresh, decision_type
...
def calculate_features(
    ...
    **v6 版本**：使用新的脂肪阈值计算逻辑（见 get_fat_threshold_per_muscle）

    Args:
        normalized_image: CSF 标准化后的图像切片（CSF 信号 ≈ 1.0）
        label: 分割标签切片
        muscle_label: 肌肉标签编号
        pixel_spacing: 像素物理间距（如果为 None，使用 (1.0, 1.0)）

    Returns:
        特征字典，包含像素间距、实际肌肉面积、脂肪阈值决策信息
    """
    ...
    # 使用 v6 改进的脂肪阈值计算（返回阈值 + 决策类型）
    if total_csa_pixels > 0:
        muscle_pixels = normalized_image[muscle_mask]
        fat_threshold, decision_type = get_fat_threshold_per_muscle(muscle_pixels)
    else:
        fat_threshold = 0.6
        decision_type = 'no_muscle_pixels'
    ...
    features['fat_threshold_decision'] = decision_type
...
def save_extraction_report(
    ...
    保存特征提取检测报告（v6 版本）
    """
    report_path = Path(output_dir) / 'extraction_report_v6.txt'

    with open(report_path, 'w', encoding='utf-8') as f:
        f.write('=' * 80 + '\n')
        f.write('肌肉特征提取检测报告 (版本 v6)\n')
...
def main():
    print('=' * 80)
    print('肌肉特征提取脚本 (版本 v6)')
    print('改进：像素值范围裁剪 [0.05, 2.0] + 脂肪阈值固定回退 0.6')
    print('=' * 80)

    # ---- 配置参数 ----
    # v6 预处理脚本输出目录（包含 CSF 标准化后的图像）
    PREPROCESSED_DIR = r"D:\dataset\yaozhui\QiLuhospital\feature_extraction_20260421dataset_prediction_results\Dataset201_LumbarMuscle_predictions800\output_v6_20260623"
    ...
    OUTPUT_DIR = r"D:\dataset\yaozhui\QiLuhospital\feature_extraction_20260421dataset_prediction_results\Dataset201_LumbarMuscle_predictions800\features_v6_20260623"
    ...
    # v6 新增：记录每位病人的脂肪阈值决策统计
    patient_threshold_stats = {}
    ...
    for patient_dir in tqdm(patient_dirs, desc='Processing patients'):
        ...
        try:
            ...
            # v6 新增：统计该病人的阈值决策类型分布
            decision_types = {}
            for feat in patient_features:
                dt = feat.get('fat_threshold_decision', 'unknown')
                decision_types[dt] = decision_types.get(dt, 0) + 1
            patient_threshold_stats[patient_id] = decision_types
    ...
    if all_features:
        ...
        output_file = Path(OUTPUT_DIR) / 'muscle_features_2d_v6.csv'
        ...
        sample_cols = ['patient_id', 'slice_index', 'muscle_name',
                      ...
                      'fat_threshold_used', 'fat_threshold_decision']
        print(df[sample_cols].head().to_string())

    if all_features_3d:
        df_3d = pd.DataFrame(all_features_3d)
        output_file_3d = Path(OUTPUT_DIR) / 'muscle_features_3d_v6.csv'
    ...
    if all_features_level3_cross:
        ...
        output_file_level3_cross = Path(OUTPUT_DIR) / 'muscle_features_level3_cross_v6.csv'
    ...
    if all_features_level3_multi:
        ...
        output_file_level3_multi = Path(OUTPUT_DIR) / 'muscle_features_level3_multi_v6.csv'
    ...
    # ---- v6 新增：保存 JSON 格式的详细日志 ----
    # 包括每位病人的脂肪阈值决策类型统计（便于后续质量控制和敏感性分析）
    log_data = {
        'timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        'version': 'v6',
        'description': 'CSF 标准化 + 像素裁剪 [0.05, 2.0] + 脂肪阈值固定回退 0.6',
        ...
        'fat_threshold_decisions_per_patient': {
            pid: {k: int(v) for k, v in stats.items()}
            for pid, stats in patient_threshold_stats.items()
        }
    }
    ...
    for pid, stats in patient_threshold_stats.items():
        for decision, count in stats.items():
            all_decision_counts[decision] = all_decision_counts.get(decision, 0) + count
    log_data['fat_threshold_decisions_total'] = {
        k: int(v) for k, v in all_decision_counts.items()
    }

    try:
        log_path = Path(OUTPUT_DIR) / 'feature_extraction_log_v6.json'
        with open(log_path, 'w', encoding='utf-8') as f:
            json.dump(log_data, f, ensure_ascii=False, indent=2)
        print(f'详细日志 JSON 已保存到: {log_path}')

        # 打印阈值决策类型的汇总统计
        print('\n脂肪阈值决策类型统计:')
        for decision_type, count in sorted(all_decision_counts.items(),
                                          key=lambda x: -x[1]):
            pct = count / sum(all_decision_counts.values()) * 100
            print(f'  {decision_type}: {count} 次 ({pct:.1f}%)')
    except Exception as e:
        print(f'警告: JSON 日志保存失败: {e}')
