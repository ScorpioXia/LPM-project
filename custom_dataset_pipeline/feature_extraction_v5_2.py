"""
肌肉特征提取脚本 (版本 v5_2)
基于 feature_extraction_v5.py 扩展，主要修改：
1. 不使用标准化后的图像进行特征提取
2. 直接使用原图（原始NIfTI图像）进行特征提取
3. 原图路径来源与 image_preprocessing_v5.py 中的 IMAGES_DIR 一致
...
输出文件：
- muscle_features_2d_v5_2.csv: 2D切片级特征（每切片每肌肉一行），基于原图
- muscle_features_3d_v5_2.csv: 3D病人级特征，基于原图
- muscle_features_level3_cross_v5_2.csv: Level 3.1跨层梯度特征，基于原图
- muscle_features_level3_multi_v5_2.csv: Level 3.2-3.5多肌肉关系特征，基于原图
- extraction_report_v5_2.txt: 特征提取检测报告
"""
...
def get_fat_threshold_per_muscle(pix_vals):
    ...
    if len(pix_vals) < 10:
        # 回退：使用默认的简单阈值
        return np.percentile(pix_vals, 75) if len(pix_vals) > 0 else 0.5
...
def calculate_features(
    ...
    注：此处的 normalized_image 参数实际传入的是原图，保留函数签名以兼容特征计算器模块

    Args:
        normalized_image: 原始图像切片（注意：实际传入原图，不使用标准化图像）
        label: 分割标签切片
        muscle_label: 肌肉标签编号
...
def process_single_patient(
    ...
    assert image_data.shape == label_data.shape, \
        f'Image shape {image_data.shape} != label shape {label_data.shape}'
    ...
    for z in range(image_data.shape[2]):
        ...
        for muscle_label, muscle_name in MUSCLE_LABELS.items():
            features = calculate_features(image_slice, label_slice, muscle_label, pixel_spacing)
...
def save_extraction_report(
    ...
    Args:
        ...
        total_patients: 总病人数
    """
    report_path = Path(output_dir) / 'extraction_report_v5_2.txt'

    with open(report_path, 'w', encoding='utf-8') as f:
        f.write('=' * 80 + '\n')
        f.write('肌肉特征提取检测报告 (版本 v5_2)\n')
        f.write('说明：不使用标准化后的图像，直接使用原图进行特征提取\n')
        f.write('=' * 80 + '\n\n')
...
def main():
    print('=' * 80)
    print('肌肉特征提取脚本 (版本 v5_2)')
    print('说明：不使用标准化后的图像，直接使用原图进行特征提取')
    print('=' * 80)

    # 配置参数 - 使用与 image_preprocessing_v5.py 相同的原图路径
    # 原图目录
    ...
    # 输出目录（区分 v5_2 版本）
    OUTPUT_DIR = r"E:\dataset\yaozhui\QiLuhospital\feature_extraction_20260421dataset_prediction_results\Dataset201_LumbarMuscle_predictions800\features_v5_2_raw"

    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    print(f'原图目录: {IMAGES_DIR}')
    ...
    # 处理每个病人
    for img_file in tqdm(image_files, desc='Processing patients'):
        # 从文件名提取病人ID（与 image_preprocessing_v5.py 第388行保持一致）
        patient_id = img_file.stem.replace('.nii', '').replace('_0000', '')
    ...
    if all_features:
        ...
        output_file = Path(OUTPUT_DIR) / 'muscle_features_2d_v5_2.csv'
    ...
    if all_features_3d:
        df_3d = pd.DataFrame(all_features_3d)
        output_file_3d = Path(OUTPUT_DIR) / 'muscle_features_3d_v5_2.csv'
    ...
    if all_features_level3_cross:
        ...
        output_file_level3_cross = Path(OUTPUT_DIR) / 'muscle_features_level3_cross_v5_2.csv'
    ...
    if all_features_level3_multi:
        ...
        output_file_level3_multi = Path(OUTPUT_DIR) / 'muscle_features_level3_multi_v5_2.csv'
    ...
    print('特征提取检测摘要')
    print('=' * 80)
    print(f'总病人数: {len(image_files)}')
    print(f'成功提取: {len(successful_patients)} ({len(successful_patients)/len(image_files)*100:.1f}%)')
    print(f'失败提取: {len(failed_patients)} ({len(failed_patients)/len(image_files)*100:.1f}%)')
